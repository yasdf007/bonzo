...
class Music(commands.Cog):
        ...
        if isinstance(event, (wavelink.TrackEnd, wavelink.TrackException)):
            ...
            if isinstance(event, wavelink.TrackException):
                await controller.channel.send(
                    f"Ошибка при проигрывании {event.player.current}"
                )
    ...
    async def cog_command_error(self, ctx, error):
        ...
        if isinstance(error, NotInVoice):
            return await ctx.send(
                embed=automata.generateEmbErr(
                    "Для использования комманды нужно быть в войсе", error=error
                )
            )
        ...
        if isinstance(error, QueueTooShort):
            return await ctx.send(
                embed=automata.generateEmbErr(
                    "Очередь слишком маленькая для перемешивания", error=error
                )
            )
        ...
        if isinstance(error, FilterInvalidArgument):
            return await ctx.send(
                embed=automata.generateEmbErr(
                    "Значение должно быть больше нуля", error=error
                )
            )
    ...
    async def on_voice_state_update(self, member, before, after):
        if member.id == self.bot.user.id:
            ...
            if (
                not self.dc_flag[member.guild.id]
                and before.channel
        ...
        if (before.channel and not after.channel) or (
            ...
            if (
                len(
                    [
                        user
                        for user in self.bot.get_channel(player.channel_id).members
                        if not user.bot
                    ]
                )
                < 1
    ...
    @commands.command(
        name="play", description="Играет музыку по ссылке или по названию"
    )
    ...
    async def play(self, ctx, query: str):
        ...
        embed = Embed(title=f"Добавил в очередь")
    ...
    async def skip(self, ctx):
        ...
        embed = Embed(title=f"Скипнул `{player.current.title}`")
    ...
    @commands.command(
        name="volume", description="Устанавливает громкость плеера", aliases=["vol"]
    )
    ...
    async def volume(self, ctx, vol: int):
        ...
        embed = Embed(title=f":speaker: Установил громкость плеера `{vol}`")
        await ctx.send(embed=embed, delete_after=15)
    ...
    @commands.command(
        name="now_playing",
        description="Показывает текущий трек",
    ...
    async def now_playing(self, ctx):
        ...
        controller.now_playing = await ctx.send(
            embed=await controller.nowPlayingEmbed(player)
        )

    @commands.command(
        name="queue",
        description="Показывает очередь из треков (первые 5)",
        aliases=["q"],
    )
    ...
    @cog_ext.cog_slash(
        name="queue", description="Показывает очередь из треков (первые 5)"
    )
    ...
    async def queue(self, ctx):
        ...
        embed = discord.Embed(title=f"В очереди - {len(upcoming)} (первые 5)")

        for song in upcoming[:5]:
            embed.add_field(
                name=song.author, value=f"[{song.title}]({song.uri})", inline=False
            )
        await ctx.send(embed=embed, delete_after=15)

    @commands.command(
        name="stop",
        description="Отключается и чистит очередь",
    ...
    @commands.command(
        name="equalizer", description="Пресеты эквалайзера музыки", aliases=["eq"]
    )
        ...
        options=[
            {
                ...
                "required": "true",
                "choices": [
                    create_choice(name="bass", value="bass"),
    ...
    async def equalizer(self, ctx, equalizer="None"):
        ...
        if not eq or not equalizer:
            ...
            return await ctx.send(
                f"Такого эквалайзера нет, доступны: {joined}", delete_after=15
            )

        await player.set_eq(eq)
        await ctx.send(
            f"Поставил эквалайзер {equalizer}. Применение займет несколько секунд...",
            delete_after=15,
        )
    ...
    @commands.command(
        name="shuffle", description="Перемешивает треки в очереди", aliases=["mix"]
    )
    ...
    @commands.command(
        name="skip_to",
        description="Пропускает до таймкода в треке (формат 0m:00s)",
        aliases=["seek"],
    )
    ...
    @cog_ext.cog_slash(
        name="skip_to", description="Пропускает до таймкода в треке (формат 0m:00s)"
    )
    ...
    async def set_speed(self, ctx: Context, speed: float):
        ...
        await ctx.send(
            f"Поставил скорость {speed}. Применение займет несколько секунд...",
            delete_after=15,
        )
    ...
    async def set_pitch(self, ctx: Context, pitch: float):
        ...
        await ctx.send(
            f"Поставил высоту {pitch}. Применение займет несколько секунд...",
            delete_after=15,
        )
    ...
    async def reset_filters(self, ctx: Context):
        ...
        await ctx.send(
            f"Убрал фильтры. Применение займет несколько секунд...", delete_after=15
        )
